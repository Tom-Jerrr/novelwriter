...
_QUICK_ACTION_FOCUS: dict[str, str] = {
...
_FOCUS_CAPABILITIES: dict[str, list[str]] = {
    "whole_book": [
        "回答当前世界模型相关的问题",
        "指出值得继续研究的设定/线索",
        "按你的要求再进入全书级检索或建议模式",
...
_FOCUS_WORKFLOW_HINTS: dict[str, str] = {
    "whole_book": (
...
def apply_quick_action_prompt(prompt: str, quick_action_id: str | None) -> str:
    """Prefix the user prompt with an internal quick-action research focus."""
    if not quick_action_id:
        return prompt
    focus = _QUICK_ACTION_FOCUS.get(quick_action_id)
    if not focus:
        return prompt
    return f"[研究重点: {focus}]\n\n{prompt}"
...
def _build_workbench_context_text(
    ...
    stage_label = _STAGE_LABELS.get(stage_key, "当前工作区")
    focus_label = _resolve_focus_label(snapshot, session_data)

    lines = [
        f"- 当前界面：{surface_label} / {stage_label}",
        f"- 当前 copilot profile：{_PROFILE_LABELS.get(snapshot.profile, snapshot.profile)}",
        f"- 当前 copilot 场景：{_FOCUS_LABELS.get(snapshot.focus_variant, _FOCUS_LABELS.get(scenario, scenario))}",
    ]
    if focus_label:
        lines.append(f"- 当前焦点：{focus_label}")
    ...
    if isinstance(focus_entity_id, int):
        lines.append(f"- 当前焦点实体 ID：{focus_entity_id}")

    capabilities = _FOCUS_CAPABILITIES.get(snapshot.focus_variant, _FOCUS_CAPABILITIES["whole_book"])
    lines.append("- 你在这个界面可做的事：")
    lines.extend(f"  {idx}. {item}" for idx, item in enumerate(capabilities, start=1))
    return "\n".join(lines)


def _build_intent_behavior_text(turn_intent: str) -> str:
...
def _build_runtime_instruction_text(snapshot: ScopeSnapshot, scenario: str) -> str:
    profile_instr = _PROFILE_INSTRUCTIONS.get(snapshot.profile, "")
    focus_key = snapshot.focus_variant or scenario
    focus_instr = _FOCUS_INSTRUCTIONS.get(focus_key, _FOCUS_INSTRUCTIONS["entity"])
...
def _format_entity_rows_for_prompt(
    ...
    for entity in entities:
        draft_tag = " [草稿]" if entity.status == "draft" else ""
        parts = [f"[Entity#{entity.id}]{draft_tag} {entity.name} ({entity.entity_type})"]
        if entity.description:
            parts.append(f"  描述: {entity.description[:300]}")
        if entity.aliases:
            parts.append(f"  别名: {', '.join(entity.aliases[:5])}")
        attrs = snapshot.attributes_by_entity.get(entity.id, [])
        for attr in attrs[:8]:
            vis = f" [{attr.visibility}]" if attr.visibility != "active" else ""
            parts.append(f"  属性 {attr.key}: {attr.surface[:200]}{vis}")
        lines.append("\n".join(parts))
    return "\n\n".join(lines)


def _format_entities_for_prompt(snapshot: ScopeSnapshot) -> str:
    return _format_entity_rows_for_prompt(snapshot.entities, snapshot)
...
def _format_relationship_rows_for_prompt(
    ...
    for relationship in relationships:
        ...
        draft_tag = " [草稿]" if relationship.status == "draft" else ""
...
def _format_relationships_for_prompt(snapshot: ScopeSnapshot) -> str:
    return _format_relationship_rows_for_prompt(snapshot.relationships, snapshot)


def _format_system_rows_for_prompt(systems: list[WorldSystem]) -> str:
    lines: list[str] = []
    for system in systems:
        draft_tag = " [草稿]" if system.status == "draft" else ""
        parts = [f"[System#{system.id}]{draft_tag} {system.name} ({system.display_type})"]
        if system.description:
            parts.append(f"  描述: {system.description[:300]}")
        if system.constraints:
            parts.append(f"  约束: {'; '.join(str(constraint)[:100] for constraint in system.constraints[:5])}")
        lines.append("\n".join(parts))
    return "\n\n".join(lines)


def _format_systems_for_prompt(snapshot: ScopeSnapshot) -> str:
    return _format_system_rows_for_prompt(snapshot.systems)
...
def _build_broad_exploration_world_overview(snapshot: ScopeSnapshot) -> str:
    entity_samples = "、".join(entity.name for entity in snapshot.entities[:6]) or "（暂无实体）"
    ...
    system_samples = "、".join(system.name for system in snapshot.systems[:4]) if snapshot.systems else "（暂无体系）"
    ...
    if relationship_samples:
        parts.append(f"关系样本：{'; '.join(relationship_samples)}")
    if snapshot.systems:
        parts.append(f"体系样本：{system_samples}")
    if draft_count:
        parts.append(
            "草稿计数："
    ...
    parts.append("默认不在首轮展开全部世界行；需要细节时请优先按需检索或展开证据。")
    return "\n".join(parts)


def _build_draft_governance_world_context(snapshot: ScopeSnapshot) -> str:
    ...
    if snapshot.draft_entities:
        parts.append(
            "### Draft 实体\n"
            + (_format_entity_rows_for_prompt(snapshot.draft_entities, snapshot) or "（暂无）")
        )
    if snapshot.draft_relationships:
        parts.append(
            "### Draft 关系\n"
            + (_format_relationship_rows_for_prompt(snapshot.draft_relationships, snapshot) or "（暂无）")
        )
    if snapshot.draft_systems:
        parts.append(
            "### Draft 体系\n"
            + (_format_system_rows_for_prompt(snapshot.draft_systems) or "（暂无）")
        )
    if supporting_entities:
        lines = [f"- {entity.name} ({entity.entity_type})" for entity in supporting_entities[:8]]
        parts.append("### 关联已确认实体（仅供定位）\n" + "\n".join(lines))
    return "\n\n".join(parts)


def _build_world_model_prompt_block(snapshot: ScopeSnapshot) -> str:
    if snapshot.profile == "broad_exploration":
        return _build_broad_exploration_world_overview(snapshot)
    if snapshot.profile == "draft_governance":
        return _build_draft_governance_world_context(snapshot)

    entities_text = _format_entities_for_prompt(snapshot) or "（暂无实体）"
    relationships_text = _format_relationships_for_prompt(snapshot) or "（暂无关系）"
    parts = [
        "### 实体",
        entities_text,
        "",
    ...
    if snapshot.systems:
        parts.extend(["", "### 体系", _format_systems_for_prompt(snapshot) or "（暂无体系）"])
    return "\n".join(parts)
...
def build_copilot_system_prompt(
    ...
    runtime_instr = _build_runtime_instruction_text(snapshot, scenario)
    workbench_context = _build_workbench_context_text(snapshot, scenario, session_data)
    intent_behavior = _build_intent_behavior_text(turn_intent)
    ...
    if interaction_locale and interaction_locale != novel_lang:
        ...
        locale_instr = f"请用小说语言（{novel_lang}）回答。"

    world_model_text = _build_world_model_prompt_block(snapshot)
    evidence_text = _format_evidence_for_prompt(evidence) or "（暂无证据）"

    if not should_preload_world_context(turn_intent):
        return f"""你是一个小说世界模型工作台助手（Copilot）。
...
"""
    ...
    return f"""你是一个小说世界模型研究助手（Copilot）。
...
def build_auto_preload(snapshot: ScopeSnapshot) -> str:
    ...
    if snapshot.profile == "focused_research":
        ...
        if focus_entity:
            parts.append(f"当前焦点实体：[Entity#{focus_entity.id}] {focus_entity.name} ({focus_entity.entity_type})")
        ...
        if entity_names:
            parts.append(f"已加载实体：{', '.join(entity_names)}")
        if relationship_lines:
            parts.append("直接关系：\n" + "\n".join(relationship_lines))
        parts.append("这个 profile 不会自动塞入全局体系和远距离实体；如需扩展，请按需调用工具。")
        return "\n".join(parts)
    ...
    if snapshot.profile == "draft_governance":
        ...
        if snapshot.draft_relationships:
            ...
            parts.append("Draft relationships:\n" + "\n".join(draft_relationships))
        if snapshot.draft_systems:
            draft_systems = [f"  [System#{system.id}] {system.name}" for system in snapshot.draft_systems[:10]]
            parts.append("Draft systems:\n" + "\n".join(draft_systems))
        parts.append("这里只有在草稿关系需要端点标签时才会额外带入 confirmed 行；请把注意力保持在草稿工作集内。")
        return "\n".join(parts)

    entity_names = [f"{entity.name}({entity.entity_type})" + (" [draft]" if entity.status == "draft" else "") for entity in snapshot.entities[:30]]
    ...
    if entity_names:
        parts.append(f"实体示例：{', '.join(entity_names[:12])}")
    if relationship_summaries:
        parts.append(f"关系示例：{'; '.join(relationship_summaries[:8])}")
    if system_names:
        parts.append(f"体系示例：{', '.join(system_names[:8])}")
    if draft_count:
        parts.append("如需草稿细节，请切到草稿治理或用工具检查具体条目。")
    parts.append("这个 profile 故意只做薄加载，请按需检索或展开证据。")
    return "\n".join(parts)
...
def build_tool_loop_system_prompt(
    ...
    if should_preload_world_context(turn_intent):
        workflow_hint = _FOCUS_WORKFLOW_HINTS.get(snapshot.focus_variant, _FOCUS_WORKFLOW_HINTS["entity"])
    else:
        workflow_hint = (
            "1. 先根据当前工作台上下文自然接话\n"
    ...
    return f"""你是一个小说世界模型研究助手（Copilot）。你可以使用工具来检索证据。
