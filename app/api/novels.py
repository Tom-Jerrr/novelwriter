...
async def continue_novel_endpoint(
    ...
    reserved_count = int(req.num_versions or 1)
    reserved = False
    try:
        try:
            reserve_quota(db, current_user.id, count=reserved_count)
            reserved = True
            ...
            if reserved:
                refund_quota(db, current_user.id, count=reserved_count)
            raise HTTPException(status_code=400, detail=str(e))
        except HTTPException:
            if reserved:
                refund_quota(db, current_user.id, count=reserved_count)
    ...
    # Reservation is based on requested num_versions; refund any unused quota just in case.
    try:
        actual = len(continuations or [])
        if reserved and actual < reserved_count:
            refund_quota(db, current_user.id, count=reserved_count - actual)
    except Exception:
        # Quota adjustment is best-effort; never fail the request after generation succeeds.
        logger.warning("Failed to adjust reserved quota after continue success", exc_info=True)
        ...
        try:
            ...
            async for event in continue_novel_stream(
                ...
                if event.get("type") == "variant_done":
                    # Deduct 1 quota per successfully completed variant.
                    # Atomic SQL update prevents going negative even under concurrency.
                    try_decrement_quota(db, current_user.id, count=1)
                    try:
                ...
                if event.get("type") == "done":
                    record_event(db, current_user.id, "generation", novel_id=novel_id, meta={"variants": len(contents_by_variant), "stream": True})
